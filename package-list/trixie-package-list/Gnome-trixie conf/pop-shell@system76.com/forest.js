...
var Forest = class Forest extends Ecs.World {
    ...
    attach_window(ext, onto_entity, new_entity, move_by, stack_from_left) {
        const right_node = Node.Node.window(new_entity);
        function swap_branches(fork) {
            const temp = fork.left;
            fork.left = fork.right;
            fork.right = temp;
        }
        const placement = (fork, left_, right_) => {
        ...
        for (const [entity, fork] of this.forks.iter()) {
            if (fork.left.is_window(onto_entity)) {
                if (fork.right) {
                    ...
                    const [left_, right_] = new_fork.is_horizontal()
